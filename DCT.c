...
int main()
{
    float **matrix_for_test = (float **) malloc(sizeof(float *)*8);
    int i,j;
    for(i = 0 ; i < 8 ; i++){
        matrix_for_test[i] = (float *) malloc(sizeof(float) * 8);
    }

    float d[8][8] =     {{34,34,34,33,34,28,35,32},
    ...
    for(i = 0; i < 8 ; i++){
        for(j = 0 ; j < 8 ; j++){
            matrix_for_test[i][j] = d[i][j];
        }
    }

    //printf("ovv");
    dct2(matrix_for_test,8);
